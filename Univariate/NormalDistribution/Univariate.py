class Univariate ():
    ...
    def Univariate(Quan,ds):
        ...
        descriptive = pd.DataFrame(index=["Mean","Median","Mode","Q1:25%","Q2:50%","Q3:75%","99%","Q4:100","IQR","1.5 Rule","Lesser Quartile",
                                         "Greater Quartile","Min Value","Max Value","Skew","Kurtosis"],columns=Quan)
        for columnName in Quan:
            ...
            descriptive.loc["Skew",columnName] = ds[columnName].skew()
            descriptive.loc["Kurtosis",columnName] = ds[columnName].kurtosis()
        return descriptive

    def CenTendency(ds,Quan):
        import pandas as pd
        Centen=pd.DataFrame(index=["Mean","Median","Mode"],columns=Quan)
        for columnName in Quan:
            Centen.loc["Mean",columnName]=ds[columnName].mean()
            Centen.loc["Median",columnName]=ds[columnName].median()
            Centen.loc["Mode",columnName]=ds[columnName].mode()[0]
        return Centen